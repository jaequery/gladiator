...
rather than opening a second, empty room. `ui/roomFlow.ts`. **Quick match** is
deliberately not one of the kinds: `?queue=1` is read separately, and only for
the `menu` intent, so asking for a stranger can never outrank a code somebody
was actually sent.
