...
/** A bot seated in `slot`, with `seed` for its stream. */
export function createBot(slot: number, seed: number): Bot {
  const rng: RngHolder = { rng: seedRng(seed) }
  const perception = createPerception(slot, rng)
  return { rng, perception, brain: createBrain(), worldModel: perception.model }
}
...
export function botCommand(bot: Bot, ground: Ground): UserCmd {
  ...
  return think(bot.brain, bot.worldModel)
}
