...
the stand-in is only reached when the buffer is empty — which is a page with
nobody on the other end of it, and will stay reachable until a room can seat two
peers (GLAD-FHKBN8). It produces `EntityState` and goes through the same
interpolation path a real snapshot pair does, and it never touches `GameState` —
so the client and the server go on agreeing about the world exactly as they did
before.
